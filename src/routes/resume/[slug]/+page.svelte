<script lang="ts">
	...
	// Helper untuk mengambil link berdasarkan label
	const getLink = (label: string) =>
		resume.links.find((link: { label: string }) => link.label.toLowerCase() === label.toLowerCase())
			?.url ?? '#';
</script>
		...
		>
			<div class="flex flex-col items-center gap-8 md:flex-row md:items-start md:gap-10">
				...
				<div class="flex-1 space-y-4 text-center md:text-left">
					...
					<p class="max-w-3xl text-xs leading-relaxed font-normal text-slate-600 sm:text-sm">
						{resume.about}
					</p>
		...
		<div class="grid grid-cols-1 gap-10 lg:grid-cols-12">
			...
			<div class="space-y-12 lg:col-span-8">
				...
				<section class="space-y-4 border-b border-slate-100 pb-10 text-left">
					...
					<div class="space-y-4 text-xs leading-relaxed font-normal text-slate-600">
						...
						{#if resume.objectives?.length}
							<div class="pt-2">
								<span class="mb-2 block font-bold text-slate-800"> My Objectives: </span>

								<ul class="space-y-2">
									{#each resume.objectives as objective}
				...
				<section class="space-y-6 border-b border-slate-100 pb-10 text-left">
					...
					<div class="relative space-y-8 border-l-2 border-slate-200 pl-6">
						{#each resume.workExperiences as exp (exp.id)}
							<div class="relative">
								<div
				...
				<section class="space-y-6 text-left">
					...
					<div class="relative space-y-8 border-l-2 border-slate-200 pl-6">
						{#each resume.educations as edu (edu.id)}
							<div class="relative">
								<div
				...
				{#if resume.additionalActivities?.length}
					<section class="space-y-6 text-left">
						<div class="flex items-center space-x-2 text-slate-900">
							<Briefcase class="h-5 w-5 text-indigo-600" />

							<h2 class="text-xl font-bold tracking-tight">Additional Activities</h2>
						</div>

						<div class="relative space-y-8 border-l-2 border-slate-200 pl-6">
							{#each resume.additionalActivities as activity (activity.id)}
								<div class="relative">
									<div
										class="absolute top-1.5 -left-[31px] h-3 w-3 rounded-full border-2 border-slate-50 bg-amber-500 shadow-sm"
									></div>

									<div class="space-y-1.5">
										<div
											class="flex flex-col gap-y-1 sm:flex-row sm:items-center sm:justify-between"
										>
											<h3 class="text-sm font-bold text-slate-900">
												{activity.position}
											</h3>
			...
			<div class="space-y-6 lg:col-span-4">
				<div class="space-y-6 rounded-3xl border border-slate-100 bg-white p-5 text-left shadow-sm">
					...
					{#each resume.skills as skillCategory}
						<div class="space-y-2">
							<span class="flex items-center text-xs font-bold text-slate-700">
								{#if skillCategory.category.toLowerCase() === 'frontend'}
									<Layout class="mr-1.5 h-3.5 w-3.5 text-pink-500" />
								{:else if skillCategory.category.toLowerCase() === 'backend'}
									<Briefcase class="mr-1.5 h-3.5 w-3.5 text-blue-500" />
								{:else if skillCategory.category.toLowerCase() === 'database'}
			...
			{#each resume.projects as project (project.id)}
				...
				>
					...
					{#if project.coverImage}
						<div
							class="relative h-56 w-full overflow-hidden border-b border-slate-100 bg-slate-100"
						>
							<img
					...
					{:else}
						<div
							class="relative flex h-56 w-full items-center justify-center border-b border-slate-100 bg-slate-100"
						>
							<Code class="h-12 w-12 text-slate-300" />
