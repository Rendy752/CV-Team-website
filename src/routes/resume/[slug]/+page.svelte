...
<div id="member-profile-page" class="min-h-screen bg-slate-50 py-8 lg:py-12 space-y-16" in:fade={{ duration: 200 }}>
	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		...
		<!-- Profile Card Header -->
		<div class="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm sm:p-10" in:fly={{ y: 20, duration: 300, delay: 100 }}>
			<div class="flex flex-col items-center gap-8 md:flex-row md:items-start md:gap-10">

				<!-- Avatar -->
				<div class="h-32 w-32 shrink-0 overflow-hidden rounded-2xl border border-slate-100 bg-slate-50 shadow-sm sm:h-44 sm:w-44">
					<img
						src={resume.avatar}
						alt={resume.name}
				...
				<div class="flex-1 space-y-4 text-center md:text-left">
					<div class="space-y-1">
						...
						<span class="block text-xs font-bold uppercase tracking-widest text-indigo-600">
							{resume.role}
						</span>
					</div>

					<p class="max-w-2xl text-xs font-normal leading-relaxed text-slate-600 sm:text-sm">
						{resume.shortIntro}
					</p>

					<!-- Location & Email -->
					<div class="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 border-t border-slate-100 pt-3 text-xs text-slate-500 md:justify-start">
		...
		<div class="grid grid-cols-1 gap-10 lg:grid-cols-12">
			...
			<div class="space-y-12 lg:col-span-8">
				...
				<section class="space-y-4 border-b border-slate-100 pb-10 text-left">
					...
					<div class="space-y-4 text-xs font-normal leading-relaxed text-slate-600">
						...
						<div class="pt-2">
							...
							<p class="italic text-slate-500">"{resume.aboutMe.goals}"</p>
						</div>
					</div>
				</section>
							...
							<div class="relative">
								...
								<div class="space-y-1.5">
									<div class="flex flex-col gap-y-1 sm:flex-row sm:items-center sm:justify-between">
										...
										<span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{exp.duration}</span>
									</div>
									<span class="text-xs font-semibold text-indigo-600">{exp.company}</span>
									<p class="text-xs font-normal leading-relaxed text-slate-600">{exp.description}</p>
								</div>
							</div>
						{/each}
					</div>
							...
							<div class="relative">
								...
								<div class="space-y-1.5">
									<div class="flex flex-col gap-y-1 sm:flex-row sm:items-center sm:justify-between">
										...
										<span class="text-[10px] font-bold uppercase tracking-wider text-slate-400">{edu.year}</span>
									</div>
			...
			<div class="space-y-6 lg:col-span-4">
				...
				<!-- Workspace File Info -->
				<div class="space-y-1 rounded-2xl border border-dashed border-slate-200 bg-white p-4 text-left text-xs leading-normal text-slate-500 shadow-sm">
					...
					<code class="mt-1 block truncate rounded border border-slate-100 bg-slate-50 p-1 font-mono text-[10px] text-slate-600">
	...
	<div class="mx-auto max-w-7xl border-t border-slate-100 px-4 pt-16 sm:px-6 lg:px-8">
		<div class="mb-10 space-y-1.5 text-left">
			<span class="block text-[10px] font-bold uppercase tracking-widest text-indigo-600">
				...
				>
					...
					<div class="relative h-56 w-full overflow-hidden border-b border-slate-100 bg-slate-100">
						<img
							...
							loading="lazy"
						/>
						<div class="absolute right-4 top-4 rounded-full border border-white/10 bg-slate-900/90 px-2.5 py-1 text-[10px] font-bold text-white backdrop-blur-sm">
							{project.status}
						</div>
					</div>
					...
					<div class="space-y-4 p-6">
						<div class="space-y-1">
							...
							<h3 class="text-xl font-bold leading-tight tracking-tight text-slate-900">
								{project.name}
							</h3>
						</div>

						<p class="line-clamp-2 text-xs font-normal leading-relaxed text-slate-600">
							{project.shortDescription}
						</p>

						<div class="flex flex-col space-y-1 rounded-xl border border-slate-100 bg-slate-50 p-3.5">
							<span class="text-[9px] font-bold uppercase tracking-wider text-slate-400">My Project Role:</span>
					...
					<div class="flex items-center justify-between border-t border-slate-100 bg-slate-50/50 px-6 py-4">
						<span class="font-mono text-[10px] text-slate-400">Duration: {project.duration}</span>
